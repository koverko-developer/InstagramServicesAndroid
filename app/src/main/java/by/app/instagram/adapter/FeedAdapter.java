...
public class FeedAdapter extends RecyclerView.Adapter {
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof FeedHolder) {
            ...
            ((FeedHolder) holder).count_likes.setText(String.valueOf(item.getCountsLikes()));
            ((FeedHolder) holder).count_comments.setText(String.valueOf(item.getCountComments()));
            if(item.getCountViews() != 0){
                ((FeedHolder) holder).count_views.setText(String.valueOf(item.getCountViews()));
